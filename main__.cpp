...
int main()
{
    auto ctx = p6::Context{{1280, 720, "TP7 EX1"}};
    ctx.maximize_window();

    EarthProgram earthProgram{};
    MoonProgram  moonProgram{};

    const img::Image imageEarth = p6::load_image_buffer("assets/textures/EarthMap.jpg");
    const img::Image imageMoon  = p6::load_image_buffer("assets/textures/MoonMap.jpg");
    const img::Image imageCloud = p6::load_image_buffer("assets/textures/CloudMap.jpg");
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageEarth.width(), imageEarth.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, imageEarth.data());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);

    glBindTexture(GL_TEXTURE_2D, textures[1]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageCloud.width(), imageCloud.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, imageCloud.data());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);

    glBindTexture(GL_TEXTURE_2D, textures[2]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageMoon.width(), imageMoon.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, imageMoon.data());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);
    ...
    ctx.update = [&]() {
        ...
        glm::mat4 earthMVMatrix = glm::translate(glm::mat4{1.f}, {0.f, 0.f, -1.f}) * glm::rotate(MVMatrix, ctx.time(), {0.f, 1.f, 0.f});
        ...
        for (int i = 0; i < 32; i++)
        {
            glm::mat4 moonMVMatrix = glm::translate(glm::mat4{1.f}, {0.f, 0.f, -1.f}) * MVMatrix;
