...
public:
    ...
    static bool generateBernoulli(float p)
    {
        return p6::random::number() < p;
    }
    ...
    static float generateLaplace(float mu, float b)
    {
        ...
        return u < 0.5f ? mu + b * std::log(2.0f * u) : mu - b * std::log(2.0f - 2.0f * u);
    }
    ...
    {
        return p6::random::direction();
    }

    static glm::vec2 generateRandomPoint(float aspect_ratio = 1.0f)
    {
        return glm::vec2(p6::random::number(-aspect_ratio, aspect_ratio), p6::random::number(-1.0f, 1.0f));
    }
};
